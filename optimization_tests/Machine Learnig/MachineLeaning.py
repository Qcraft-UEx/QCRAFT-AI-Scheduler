...
def optimizar_espacio_ml(model, queue, capacidad, forced_threshold=FORCE_THRESHOLD, alpha=1.0, lambda_penalty=0.5):
    ...
    features = []
    for (ident, valor, count) in queue_restante:
        features.append([valor / float(capacidad), count / float(forced_threshold)])
    if features:
        input_tensor = torch.tensor([features], dtype=torch.float32)  # forma: (1, n_items, 2)
        with torch.no_grad():
            probs = model(input_tensor)
        probs = probs.squeeze(0).tolist()  # Lista de probabilidades
    else:
        probs = []

    # Ajustar bonus si hay muchos elementos de alta prioridad en los restantes
    ...
    # Asociar a cada elemento restante su probabilidad y calcular profit:
    ...
    # 3. Aplicar knapsack para los elementos restantes con la capacidad restante
